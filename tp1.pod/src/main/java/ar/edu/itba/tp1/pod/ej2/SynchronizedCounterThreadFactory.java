...
public class SynchronizedCounterThreadFactory {

	public static SynchronizedCounterThread[] sysoThreadArray(int length,
			final String thread_name, final String message, int times) {
		SynchronizedCounterThread[] threads = new SynchronizedCounterThread[length];
		for (int i = 0; i < length; i++)
			threads[i] = new SynchronizedCounterThread(times, thread_name, message);
		return threads;
	}
	...
	public static SynchronizedCounterThread[] sysoThreadArray(int length,
		...
		SynchronizedCounterThread[] threads = new SynchronizedCounterThread[length];
		for (int i = 0; i < length; i++)
			threads[i] = new SynchronizedCounterThread(times, thread_names[i], messages[i]);
		return threads;
	}
	...
	public static SynchronizedCounterThread[] sysoThreadArray(int length,
		...
		SynchronizedCounterThread[] threads = new SynchronizedCounterThread[length];
		for (int i = 0; i < length; i++)
			threads[i] = new SynchronizedCounterThread(thread_names[i], messages[i],
					sleep_times[i]);
		return threads;
	}
	...
	public static SynchronizedCounterThread[] sysoThreadArray(int length, int times) {
		return sysoThreadArray(length, defaultNames(length),
				defaultMessages(length), times);
	}

	public static SynchronizedCounterThread[] sysoThreadArray(int length, int[] sleep_times) {
